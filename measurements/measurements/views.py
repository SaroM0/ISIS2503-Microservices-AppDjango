...
def check_place(data):
    place_id = data["place"]
    r = requests.get(f'{settings.PLACES_SERVICE_URL}/places/{place_id}/', headers={"Accept":"application/json"})
    ...
    if request.method == 'POST':
        ...
        for measurement in data_json:
            if check_variable(measurement) and check_place(measurement):
                ...
                return HttpResponse("unsuccessfully created measurements. Variable or Place does not exist")
